            ...
            res.hijack(function (err, res) {
                ...
                if (matchContentType || (/\.less(?:\?.*)?$/.test(req.url) && contentType === 'application/octet-stream')) {
                    function sendErrorResponse(errorMessage, cssText) {
                        ...
                        res.send("body * {display: none !important;} body:before {line-height: 1.5; display: block; z-index: 99999999; white-space: pre; font-family: 'Courier New', monospace; font-size: 24px; color: black; margin: 10px; padding: 10px; border: 4px dashed red; margin-bottom: 10px; content: '" + errorMessage.replace(/'/g, "\\'").replace(/\n/g, '\\00000a') + "'}\n" + (cssText || ''));
                    }
                            ...
                            seq(Object.keys(isSeenByFileName))
                                ...
                                .seq(function (fileNames) {
                                    ...
                                    try {
                                        ...
                                        sendErrorResponse(err.message + ('line' in err ? ' at line ' + err.line + ('column' in err ? ', column ' + err.column : '') : '') +
                                                          ('extract' in err ? ':\n' + err.extract.join('\n') : ''));
                                    }
                               });
                        });
